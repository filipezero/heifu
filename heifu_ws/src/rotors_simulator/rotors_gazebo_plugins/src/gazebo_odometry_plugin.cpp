...
GazeboOdometryPlugin::~GazeboOdometryPlugin() {
  //event::Events::stop(updateConnection_);
  if (node_handle_) {
...
void GazeboOdometryPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  ignition::math::Pose3d asd;


  math::Vector3d noise_normal_position;
  math::Vector3d noise_normal_quaternion;
  math::Vector3d noise_normal_linear_velocity;
  math::Vector3d noise_normal_angular_velocity;
  math::Vector3d noise_uniform_position;
  math::Vector3d noise_uniform_quaternion;
  math::Vector3d noise_uniform_linear_velocity;
  math::Vector3d noise_uniform_angular_velocity;
  const math::Vector3d zeros3(0.0, 0.0, 0.0);
...
void GazeboOdometryPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  math::Pose3d W_pose_W_C = link_->WorldCoGPose();
  math::Vector3d C_linear_velocity_W_C = link_->RelativeLinearVel();
  math::Vector3d C_angular_velocity_W_C = link_->RelativeAngularVel();

  math::Vector3d gazebo_linear_velocity = C_linear_velocity_W_C;
  math::Vector3d gazebo_angular_velocity = C_angular_velocity_W_C;
  math::Pose3d gazebo_pose = W_pose_W_C;

  if (parent_frame_id_ != kDefaultParentFrameId) {
    math::Pose3d W_pose_W_P = parent_link_->WorldPose();
    math::Vector3d P_linear_velocity_W_P = parent_link_->RelativeLinearVel();
    math::Vector3d P_angular_velocity_W_P = parent_link_->RelativeAngularVel();
    math::Pose3d C_pose_P_C_ = W_pose_W_C - W_pose_W_P;
    math::Vector3d C_linear_velocity_P_C;
